...
final class StubbornSender {
  ...
  // Sends a message (one time only) without tracking. Used for control messages (e.g., ACKs) that are not retried.
  ...
  void cancelTracked(TrackedKey key, InetSocketAddress remoteEndpoint) {
    ...
    synchronized (context.stateLock) {
      ...
      TrackedTargetKey targetKey = new TrackedTargetKey(remoteEndpoint, key);
      if (context.retryRegistry.removeTracked(targetKey) == null) { // No tracked was removed, so record a pending cancel to prevent future registration (race condition).
